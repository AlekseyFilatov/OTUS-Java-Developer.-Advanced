...
import otus.cqrses.cqrsesproj.application.exceptions.EventStoreRuntimeException;
...
import static otus.cqrses.cqrsesproj.application.event.Constants.*;
...
public class EventStore implements EventStoreDB {
    ...
    private void eventsBatchInsert(@SpanTag("events") List<Event> events) {
        final List<Map<String, Serializable>> args = events.stream().map(this::mapFromEvent).toList();
    ...
    private <T extends AggregateRoot> T getAggregate(@SpanTag("aggregateId") final String aggregateId, @SpanTag("aggregateType") final Class<T> aggregateType) {
        try {
            ...
            throw new EventStoreRuntimeException(aggregateId, ex);
        }
    }
